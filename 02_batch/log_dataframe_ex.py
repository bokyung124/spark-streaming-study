...
if __name__ == "__main__":
    ss: SparkSession = SparkSession.builder.master("local").appName("log dataframe ex").getOrCreate()

    # define schema
    fields = StructType(
        [
            StructField("ip", StringType(), False),  # 마지막 요소: Nullable 여부
    ...
    df.groupby(group_cols).agg(
        max("latency").alias("max_latency"), min("latency").alias("min_latency"), mean("latency").alias("mean_latency")
    )
    ...
    ).groupby(group_cols).agg(collect_set("ip").alias("ip_list"), count("ip").alias("ip_count")).sort(
        group_cols
