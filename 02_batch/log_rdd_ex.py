...
if __name__ == "__main__":
    ss: SparkSession = SparkSession.builder.master("local").appName("rdd examples ver").getOrCreate()
    ...
    rdd_count_by_minute = parsed_log_rdd.map(extract_hour_and_minute).reduceByKey(lambda c1, c2: c1 + c2).sortByKey()
    ...
    # groupby 사용
    parsed_log_rdd.map(extract_cols).map(lambda x: ((x[0], x[1]), x[2])).groupByKey().mapValues(
        list
